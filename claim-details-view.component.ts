...
@Component({
  selector: 'app-claim-details-view',
  templateUrl: './claim-details-view.component.html',
  styleUrls: ['./claim-details-view.component.scss']
})
export class ClaimDetailsViewComponent implements OnInit {
  ...
  availConcessionLazyComp: Promise<Type<AvailConcessionComponent>>;
  availableDiscountLazyComp: Promise<Type<AvailableDiscountComponent>>;
  actualConcessionCRMLazyComp: Promise<Type<ActualConcessionCrmComponent>>;
  crmCreditAmountLazyComp: Promise<Type<CrmCreditAmountComponent>>;
        ...
        setTimeout(() => {
          if(this.sharedService.claimObjById && this.sharedService.claimObjById != null && this.sharedService.claimObjById != ""){
            ...
            setTimeout(() => {
              ...
              this.claimDetailsViewForm.controls.GDC_APPROVED_AMOUNT.setValue(this.claimDetailsObj.claim_AMT_GDC);
  ...
  createOrEditForm() {
    this.claimDetailsViewForm = this.formBuilder.group({
      ...
      GDC_APPROVED_AMOUNT: [this.claimDetailsObj?.claim_AMT_GDC],
      AVAILABLE_DISCOUNT: [''],
      ACTUAL_DISCOUNT_SAP: [''],
      USE_DISCOUNT: [''],
      DEALER_SHARE: [''],
  ...
  setClaimDetails(){
    ...
    this.sharedService.changeChassisNo(this.claimDetailsViewForm.controls.CHASSIS_NO.value);
  ...
  openAvailConcession(){
    setTimeout(() => {
      jQuery('#model_avail_concession_external').modal('setting', 'closable', false).modal('setting', 'autofocus', false).modal('show');

      this.availConcessionLazyComp = import('../avail-concession/avail-concession.component').then(({AvailConcessionComponent}) => AvailConcessionComponent);
    }, 0);
  }

  closeAvailConcession(){
    jQuery('#model_avail_concession_external').modal('setting', 'closable', false).modal('setting', 'autofocus', false).modal('hide');

    this.availConcessionLazyComp = null;
  }

  openAvailableDiscount(){
    setTimeout(() => {
      jQuery('#model_available_discount_external').modal('setting', 'closable', false).modal('setting', 'autofocus', false).modal('show');

      this.availableDiscountLazyComp = import('../available-discount/available-discount.component').then(({AvailableDiscountComponent}) => AvailableDiscountComponent);
    }, 0);
  }

  closeAvailableDiscount(){    
    jQuery('#model_available_discount_external').modal('setting', 'closable', false).modal('setting', 'autofocus', false).modal('hide');

    this.availableDiscountLazyComp = null;
  }

  openCRMCreditAmount(){
    setTimeout(() => {
      jQuery('#model_crm_credit_amount_external').modal('setting', 'closable', false).modal('setting', 'autofocus', false).modal('show');

      this.crmCreditAmountLazyComp = import('../crm-credit-amount/crm-credit-amount.component').then(({CrmCreditAmountComponent}) => CrmCreditAmountComponent);
    }, 0);
  }

  closeCRMCreditAmount(){
    jQuery('#model_crm_credit_amount_external').modal('setting', 'closable', false).modal('setting', 'autofocus', false).modal('hide');

    this.crmCreditAmountLazyComp = null;
  }

  openActualConcessionCRM(){
    setTimeout(() => {
      jQuery('#model_actual_concession_crm_external').modal('setting', 'closable', false).modal('setting', 'autofocus', false).modal('show');

      this.actualConcessionCRMLazyComp = import('../actual-concession-crm/actual-concession-crm.component').then(({ActualConcessionCrmComponent}) => ActualConcessionCrmComponent);
    }, 0);
  }

  closeActualConcessionCRM(){
    jQuery('#model_actual_concession_crm_external').modal('setting', 'closable', false).modal('setting', 'autofocus', false).modal('hide');

    this.actualConcessionCRMLazyComp = null;
  }
