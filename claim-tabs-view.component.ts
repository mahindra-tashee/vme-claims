...
@Component({
  selector: 'app-claim-tabs-view',
  templateUrl: './claim-tabs-view.component.html',
  styleUrls: ['./claim-tabs-view.component.scss']
})
export class ClaimTabsViewComponent implements OnInit {
  ewDetailsLazyComp: Promise<Type<EwDetailsViewComponent>>;
  amcDetailsLazyComp:Promise<Type<AmcDetailsViewComponent>>;
  claimsDetailsViewLazyComp:Promise<Type<ClaimDetailsViewComponent>>;
  documentsViewLazyComp: Promise<Type<DocumentViewComponent>>;
  ...
  ngOnInit(): void {
    ...
    this.sharedService.messageKey = "claimDashboardMessage";
    ...
    this.sharedService.getClaimIdNotAvailable().subscribe((response) => {
      if(response && response == true){
        ...
        setTimeout(() => {
          this.messageService.add({key: this.sharedService.messageKey, severity:'info', summary: 'Note', detail:'Please Create Claim', life: 7000});
        }, 0);
  ...
  getBasicClaimsDetails(chassisNo, schemeId, claimId){
    this.claimsService.getApproverIndividualBasicClaimsDetails(chassisNo, schemeId, '', claimId).subscribe((claimsDashboardResponse) => {
      if(claimsDashboardResponse && claimsDashboardResponse.STATUS_OUT === "SUCCESS" && claimsDashboardResponse.RESPONSE_OUT != null && claimsDashboardResponse.RESPONSE_OUT != ""){
        ...
        this.messageService.add({key: this.sharedService.messageKey, severity:'info', summary: 'Note', detail:'No Data Available for Claim Details By Chassis', life: 7000});
      }
    }, (error) => {
      $('html,body,div').animate({ scrollTop: 0 }, 'slow');
      if(error && error.status && error.status == 400 && error.error && error.error.errors && error.error.errors[0]){
        ...
        this.messageService.add({key: this.sharedService.messageKey, severity:'error', summary: 'Error', detail:'Error Getting Claim Details By Chassis', life: 7000});
      }
    });
  }
  ...
  getClaimDataById(claimId, flag){
    ...
    this.claimsService.getApproverClaimDetailsById(claimId).subscribe((claimTabsViewResponse) => {
      if(claimTabsViewResponse && claimTabsViewResponse.STATUS_OUT == "SUCCESS" && claimTabsViewResponse.RESPONSE_OUT != null){
        ...
        setTimeout(() => {
          this.claimsDetailsViewLazyComp = import('../claim-details-view/claim-details-view.component').then(({ClaimDetailsViewComponent}) => ClaimDetailsViewComponent);
          this.documentsViewLazyComp = import('../document-view/document-view.component').then(({DocumentViewComponent}) => DocumentViewComponent);
        ...
        $('html,body,div').animate({ scrollTop: 0 }, 'slow');
        this.messageService.add({key: this.sharedService.messageKey, severity:'error', summary: 'Error', detail:'Error Getting Individual Claim Data', life: 7000});
      }
    }, (error) => {
      $('html,body,div').animate({ scrollTop: 0 }, 'slow');
      if(error && error.status && error.status == 400 && error.error && error.error.errors && error.error.errors[0]){
